...
* `--modern` pins the **Debian-sid** packages for `gcc-avr-14` (falling back to
  Ubuntu’s `gcc-avr` if sid is unreachable).  It installs QEMU, Meson and
  documentation helpers, compiles a demo firmware and boots it in
  QEMU (`arduino-uno`).  CFLAGS default to **C23**.
